...
export default function FoodPage() {
  ...
  useEffect(() => {
    ...
    getById(id).then((fetchedFood) => {
      // Clone the fetchedFood to avoid mutating the original data
      const foodClone = { ...fetchedFood };

      // Set default values for size and addons if they exist
      if (foodClone.size) {
        foodClone.selectedOptions = foodClone.size[0];
        foodClone.name = foodClone.id + ` - (${foodClone.size[0]})`;
      }
      ...
      // Update the state with the cloned and modified data
      setFood(foodClone);
    });
  }, [id]);
