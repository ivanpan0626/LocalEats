...
export default function CheckoutPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex justify-center max-w-7xl mx-auto flex-wrap mb-24">
      {/* Cart Section */}
        ...
        {cart.items.length === 0 ? (
          ...
          <div className="flex flex-col border border-red-300 rounded-lg p-4 h-96 custom-scroll overflow-y-auto flex-grow">
            <ul className="space-y-4">
              {cart.items.map((item) => (
                <li key={item.food.id} className="flex justify-between items-center py-4 border-b border-gray-200">
                  <img
                  ...
                  <div className="flex flex-col w-full">
                    <div className="text-left px-2 text-ellipsis overflow-hidden">{item.food.name}</div>
      ...
      <div className="w-full sm:w-96">
        ...
        <div className="space-y-4">
          ...
          {orderType === "pickup" ? (
            <>
              <Input label="Name" {...register("name", { required: "Name is required" })} error={errors.name} />
              <Input label="Phone Number" {...register("phone", { required: "Phone number is required" })} error={errors.phone} />
              <div>
            ...
            <>
              <Input label="Name" {...register("name", { required: "Name is required" })} error={errors.name} />
              <Input label="Phone Number" {...register("phone", { required: "Phone number is required" })} error={errors.phone} />
              <Input label="Address" {...register("address", { required: "Address is required" })} error={errors.address} />
              <Input label="APT#" {...register("apt")} error={errors.apt} />
              <Input label="City" {...register("city", { required: "City is required" })} error={errors.city} />
              <Input label="Zip Code" {...register("zipcode", { required: "Zip Code is required" })} error={errors.zipcode} />
              <div>
          ...
          <div className="flex justify-between items-center my-4">
            <div className="flex items-center">
              ...
              <div className="relative group">
                <Info size={18} className="inline-block ml-2 cursor-pointer text-gray-500" />
          ...
          {/* Total Price */}
          <div className="flex justify-between items-center my-4">
            Total: <Price price={cart.totalPrice + cart.totalPrice * 0.06625 + 0.99} />
          </div>

          {/* Submit Button */}
          <Button type="submit" className="w-full mt-8 py-3 rounded-md bg-red-600 text-white">
            Proceed to Payment
          </Button>
        </div>
      </div>
