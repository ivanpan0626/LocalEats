...
export default function CheckoutPage() {
  ...
  const checkIfPast9PMEST = () => {
    ...
    // Convert to EST by subtracting 5 hours from UTC (adjust for daylight saving if needed)
    const estOffset = now.getTimezoneOffset() + 420; // 300 minutes = 5 hours
    const estTime = new Date(now.getTime() - estOffset * 60000);

    // Set a reference time for 9:30 PM in EST
    const nineThirtyPMEST = new Date(estTime);
    nineThirtyPMEST.setHours(9, 0, 0, 0); // 9:30 PM EST

    // Check if the current time is past 9:30 PM EST
    console.log(estTime, nineThirtyPMEST);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.data && response.data.sessionId) {
        ...
        alert(
          "Site under maintenance! Please call 201-384-1880 to place your order!"
        );
      } else {
